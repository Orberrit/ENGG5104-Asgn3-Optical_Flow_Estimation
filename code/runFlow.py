...
from estimateHSflow import * # include this package if you finish it
...
#    img1 = cv2.imread('./data/other-data/Venus/frame10.png').astype('single')
#    img2 = cv2.imread('./data/other-data/Venus/frame11.png').astype('single')
    # input ground truth optical flow
    flow_gt = ft.readFlowFile('./data/other-gt-flow/RubberWhale/flow10.flo')
#    flow_gt = ft.readFlowFile('./data/other-gt-flow/Venus/flow10.flo')
    ...
    start = time.time()
    uv = estimateHSflow( img1g, img2g, lam ) 
    end = time.time()
    print 'Runtime: %3.3fs \n' % (end - start)
    #uv = flow_gt
    ...
    plt.subplot(223), plt.imshow(ft.flowToColor(
        uv,-1)[:, :, [0, 1, 2]].astype('uint8')), plt.title('Flow')
